...
$('#frmmenu').submit(function(){
  ...
  if (result.isConfirmed) {
   /*swal("¡Poof, se ha guardado correctamente!", {
    ...
    $.ajax({
              type: "POST",
              url: "../controlador/controllermenu.php?btnguardar=guardar", //esto es el mismo boton guardar del controlador =>($page = isset($_GET['btnGuardar'])?$_GET['btnGuardar']:'';)
              data: formData,
              cache: false,
              contentType: false,
              processData: false,
...
$('#frmmenu2').submit(function(){
   ...
   /*swal("¡Poof, se ha modificado correctamente!", {
            ...
            var formData = new FormData(document.getElementById("frmmenu2"));
            formData.append("dato", "valor");
            $.ajax({
                type: "POST",
                url: "../controlador/controllermenu.php?btnmodificar=modificar", //esto es el mismo boton modificar del controlador =>($page = isset($_GET['btnModificar'])?$_GET['btnModificar']:'';)
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
...
function consultarcombo() {
    $.ajax({
        type: "POST",
            url: "../controlador/controllermenu.php?btnconsultar=consultarcb",//hasta para consultar tenemos un boton imaginario en el controlador  => ($page = isset($_GET['btnConsultar'])?$_GET['btnConsultar']:'';)
...
/*function consultarcombo2() {
 ...
 function llenarCajas(txt,txt2,cmb){//funcion para mandar datos de la tabla al segundo modal
    document.frmmenu2.txtidmenu2.value=txt;
    document.frmmenu2.txtvalor2.value=txt2;
...
// suspender
function suspender(idmenu,estado,valor) {
  Swal.fire({
        position: 'top-end',
   ...
   /*swal("¡Poof, se ha eliminado correctamente!", {
      icon: "success",
          ...
          var paren = idmenu
          var dataString = 'idmenu='+paren+'&estado='+estado;
           $.ajax({
            type: "POST",
            url: "../controlador/controllermenu.php?btnsuspender=suspender",//esto es el mismo boton suspender que esta en el controlador =>($page = isset($_GET['btnsuspender'])?$_GET['btnsuspender']:'';)
            //data: dataString,
            ...
            success: function(resp) {
              ...
              if (resp=="-1") {
                ...
                Swal.fire({position: 'top-end',
                  ...
                  title: estado==1?"¡Excelente!, Menu activado correctamente":"¡Excelente!, Menu desactivado correctamente",
                  showConfirmButton: false
                });
                consultar("");
              }
...
function consultar(dato) {
    ...
    "ajax": {
      url: "../controlador/controllermenu.php?btnconsultar=consultar",//hasta para consultar tenemos un boton imaginario en el controlador  => ($page = isset($_GET['btnConsultar'])?$_GET['btnConsultar']:'';)
          "type": "POST"
    },
    "columns": [
      { "data": "valor" },
