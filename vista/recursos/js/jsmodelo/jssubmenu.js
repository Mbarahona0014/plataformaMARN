...
$('#frmsubmenu').submit(function(){
  ...
  if (result.isConfirmed) {
   /*swal("¡Poof, se ha guardado correctamente!", {
      ...
      var formData = new FormData(document.getElementById("frmsubmenu"));
      formData.append("dato", "valor");
    $.ajax({
              type: "POST",
              url: "../controlador/controllersubmenu.php?btnguardar=guardar", //esto es el mismo boton guardar del controlador =>($page = isset($_GET['btnGuardar'])?$_GET['btnGuardar']:'';)
              data: formData,
              cache: false,
              contentType: false,
              processData: false,
              ...
              if (resp=="1") {
                ...
                console.log(resp);
                //swal("¡Error al guardar datos!", "Un dato no ha de concordar con el resto, verifique."+resp, "error");
                //limpiar();
              }
...
$('#frmsubmenu2').submit(function(){
   ...
   /*swal("¡Poof, se ha modificado correctamente!", {
            ...
            var formData = new FormData(document.getElementById("frmsubmenu2"));
            formData.append("dato", "valor");
            $.ajax({
                  type: "POST",
                  url: "../controlador/controllersubmenu.php?btnmodificar=modificar", //esto es el mismo boton modificar del controlador =>($page = isset($_GET['btnModificar'])?$_GET['btnModificar']:'';)
                  data: formData,
                  cache: false,
                  contentType: false,
                  processData: false,
...
function consultarcombo() {
    $.ajax({
        type: "POST",
            url: "../controlador/controllersubmenu.php?btnconsultar=consultarcb",//hasta para consultar tenemos un boton imaginario en el controlador  => ($page = isset($_GET['btnConsultar'])?$_GET['btnConsultar']:'';)
            data: 'btnconsultar=consultarcb'
               ...
               html="<option value=''>Seleccione el tipo de menu</option>";
               for (var i = 0; i< valores.length; i++) {
                   html+="<option value='"+valores[i][0]+"'>"+valores[i][1]+"/"+valores[i][2]+"</option>"
               }
               $("#tpmenu").html(html);//colocamos todo nuestro codigo HTML en nuestra pagina con esta linea, aunque si se fijan concatenamos un par de variables
...
/*function consultarcombo2() {
 ...
 function llenarCajas(txt,txt2,txt3,cmb){//funcion para mandar datos de la tabla al segundo modal
    document.frmsubmenu2.txtidsubmenu2.value=txt;
    document.frmsubmenu2.txtvalor2.value=txt2;
    document.frmsubmenu2.txtvista2.value=txt3;
...
// suspender
function suspender(idsubmenu,estado,valor) {
  Swal.fire({
        position: 'top-end',
        title: estado==1?"¿Activar?":"¿Suspender?",
        text: estado==1?"¿Desea activar "+valor+"?" :"¿Desea suspender "+valor+"?",
        icon: "warning",
        toast: true,
        showCancelButton: true,
        confirmButtonText: 'Sí, por favor.',
   ...
   /*swal("¡Poof, se ha eliminado correctamente!", {
      icon: "success",
          ...
          var paren = idsubmenu
          var dataString = 'idsubmenu='+paren+'&estado='+estado;
           $.ajax({
            type: "POST",
            url: "../controlador/controllersubmenu.php?btnsuspender=suspender",//esto es el mismo boton suspender que esta en el controlador =>($page = isset($_GET['btnsuspender'])?$_GET['btnsuspender']:'';)
            //data: dataString,
            ...
            success: function(resp) {
             ...
             if(resp=="1"){
                Swal.fire({position: 'top-end',
                  ...
                  title: estado==1?"¡Excelente!, Sub-menu activado correctamente":"¡Excelente!, Sub-menu desactivado correctamente",
                  showConfirmButton: false
                });
                consultar("");
              }else{
                //swal("¡Error al suspender datos!", "Un dato no ha de concordar con el resto, verifique."+resp, "error");
                Swal.fire({position: 'top-end',
                      icon: 'error',
                      toast: true,
                      timer: 3000,
...
function consultar(dato) {
    ...
    "ajax": {
      url: "../controlador/controllersubmenu.php?btnconsultar=consultar",//hasta para consultar tenemos un boton imaginario en el controlador  => ($page = isset($_GET['btnConsultar'])?$_GET['btnConsultar']:'';)
          "type": "POST"
    },
    "columns": [
      { "data": "valor" },
...
function limpiar()
{
  document.getElementById("frmsubmenu").reset();
  consultarcombo();
