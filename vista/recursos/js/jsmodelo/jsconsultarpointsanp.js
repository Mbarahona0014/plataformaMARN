  ...
  require([
    ...
    const clusterConfig = {
      ...
      popupTemplate: {
        ...
        content:
          "{cluster_count} evaluaciones de efectividad de manejo para esta ANP.",
        fieldInfos: [
          {
    ...
    $("#tpmostrar").change(function () {
      ...
      if (!($("#tpmostrar").val() == "all")) {
        if ((cod = document.getElementById("tpmostrar").value == "poligonos")) {
          ...
          (cod = document.getElementById("tpmostrar").value == "puntos")
        ) {
          /* map.remove(layer2); */
          map.add(layer);
        } else if (
          (cod = document.getElementById("tpmostrar").value == "todos")
        ) {
          map.remove(layer);
          /* map.remove(layer2); */
          map.add(layer);
    ...
    view.center = [-88.2, 13.7]; // Sets the center point of the view at a specified lon/lat
    view.zoom = 9;
    ...
    for (var i = 1; i < 5; i++) {
      if (i == 1) {
        html +=
          '<input type="checkbox" name="option1" onclick="DATOS(' +
          i +
          ');" id="option1" value="Yes"> <span class="opcion">Sitios Ramsar</span><br>';
      }
      if (i == 2) {
        html +=
          '<input type="checkbox" name="option2" onclick="DATOS(' +
          i +
          ');" id="option2" value="Yes"> <span class="opcion">Areas Naturales Protegidas</span> <br>';
      }
      if (i == 3) {
        html +=
          '<input type="checkbox" name="option3" onclick="DATOS(' +
          i +
          ');" id="option3" value="Yes"> <span class="opcion">Areas de conservación</span> <br>';
      }
      if (i == 4) {
        html +=
          '<input type="checkbox" name="option4" onclick="DATOS(' +
          i +
          ');" id="option4" value="Yes"> <span class="opcion">Reserva biosfera</span> <br>';
      }
    }
...
var DATOS = (i) => {
  ...
  require([
    ...
    var view = new MapView({
      container: "viewDiv",
      map: map,
      center: [-88.2, 13.7], //Longitude, latitude
      zoom: 8,
    });
