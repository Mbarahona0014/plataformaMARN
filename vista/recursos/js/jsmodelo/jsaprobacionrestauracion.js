...
var d = ()=>{
  if(localStorage.getItem('tagsi')!=null){
    ...
    $("#lista").html('<div class="alert alert-warning">Ninguna especie ingresada aún...</div>');
  }else{
    $("#lista").html('<div class="alert alert-warning">Ninguna especie ingresada aún...</div>');
  }
}
...
.then((result) => {
  if (result.isConfirmed) {
    ...
    //////////////////////////////////////
 	$.ajax({
            type: "POST",
            url: "../controlador/controllerrestauracionpuntos.php?btnmodificar=modificar&txtlistaespecie="+textlista, //esto es el mismo boton modificar del controlador =>($page = isset($_GET['btnmodificar'])?$_GET['btnmodificar']:'';)
...
/*  tags.sort((a,b)=>{return b.indice-a.indice});
  ...
  if (html=='') {
    $("#lista").html('<div class="alert alert-info">Ninguna especie ingresada aún...</div>');
  }else{
    //alert(tagasmemoria);
    $("#lista").html(html);
  }
        ...
        success: function(resp) {            
           ...
           if (resp=="3") {
            ...
            Swal.fire({position: 'top-end',
              ...
              title:"Este registro ya fue procesado por otro usuario",
              showConfirmButton: false
            });
            consultar("");
          }
...
function aprobarregistro(id,estado,tecnica,nombreexterno,apellidoexterno,correo) {
  //alert(id);
           ...
           if (resp=="3") {
            ...
            //alert(resp);
              Swal.fire({
                  position: 'top-end',
                  title: "¿Aprobar?",
                  html: "¿Desea aprobar "+tecnica+"?, se notificará a: <b>"+nombreexterno+" "+apellidoexterno+"</b> con el correo: <b>"+correo+"</b> que su registro ha sido aprobado.",
                  icon: "warning",
                  toast: true,
                  showCancelButton: true,
                  confirmButtonText: 'Sí, por favor.',
                  ...
                  $.ajax({
                    type: "POST",
                    url: "send-mailer.php",
                    //ACCION 4 SIGNIFICA NOTIFICAR APROBACION
                    //data: "correo="+correo+"&accion=4",
                    data: {correo:correo,accion:4},
                    success: function(resp) {
                      ...
                      if(resp=="1")
                      {
                        ...
                        var dataString = 'id='+paren+'&estado='+estado;
                         $.ajax({
                          type: "POST",
                          url: "../controlador/controllerrestauracionpuntos.php?btnsuspender=suspender",//mismo metodo que suspender, ya que pasara el estado de 3 a 1
                          //data: dataString,
                          ...
                          success: function(resp) {            
                             ...
                             if (resp=="-1") {
                                  Swal.fire({position: 'top-end',
                                    ...
                                    title: '¡Error al aprobar datos!, un dato no ha de concordar con el resto, verifique.',
                                    showConfirmButton: false
                                  });
                            }else if(resp=="1"){
                              $("[data-dismiss=modal]").trigger({ type: "click" });
                              Swal.fire({position: 'top-end',
                                ...
                                title:"¡Excelente!, aprobado correctamente",
                                showConfirmButton: false
                              });
                              consultar("");
                            }
                        ...
                        Swal.fire({position: 'top-end',
                          ...
                          title: '¡Upps!, no se pudo enviar el correo de notificación...',
                          showConfirmButton: false
                        });
                      }
            ...
            Swal.fire({position: 'top-end',
              ...
              title:"Este registro ya fue verficicado por otro usuario",
              showConfirmButton: false
            });
            consultar("");
          }
...
function rechazarregistro(id,estado,tecnica,nombreexterno,apellidoexterno,correo) {
  //alert(estado);
           ...
           if (resp=="3") {
            ...
            //alert(resp);
              Swal.fire({
                  position: 'top-end',
                  title: "¿RECHAZAR?",
                  html: "¿Desea rechazar "+tecnica+"?, se notificará a: <b>"+nombreexterno+" "+apellidoexterno+"</b> con el correo: <b>"+correo+"</b> que su registro ha sido rechazado.",
                  icon: "warning",
                  toast: true,
                  showCancelButton: true,
                  confirmButtonText: 'Sí, por favor.',
                  ...
                  if (result.isConfirmed) {
                    ...
                    alertarmotivo(id,correo,estado)

                    //AQUI LA FUNCION ASYNC DEL MENSAJE
                  ...
                  $.ajax({
                    type: "POST",
                    url: "send-mailer.php",
                    //ACCION 5 SIGNIFICA NOTIFICAR RECHAZO
                    data: "correo="+correo+"&accion=5",
                    success: function(resp) {
  ...
  $.ajax({
            ...
            data: 'btnconsultar=consultararchivomodifica'
               ...
               for (var i = 0; i< valores.length; i++) {
                  ...
                  html+="<img src='../vista/recursos/images/restauracion/"+valores[i][1]+"'  class='img-responsive'><br><button class='btn btn-danger' onclick='eliminarImagen("+valores[i][0]+","+valores[i][2]+")'>ELIMINAR&nbsp;<i class=\"fa fa-trash\"  style=\"font-size: 15px;\" aria-hidden=\"true\"></i></button>";
                  html+="</div>";
...
function consultar(dato) {
    ...
    "ajax": {
      "url": "../controlador/controllerrestauracionpuntos.php?btnconsultar=consultaraprobacion",
          "type": "POST"
    },          
    "columns": [
      { "data": "acciones" },
...
async function alertarmotivo(id,correo,estado){
  const { value: motivo } = await Swal.fire({
    ...
    inputValidator: motivo => {
            ...
            if (!motivo) {
                return "Por favor escribe el motivo de rechazo";
            } else {
                return undefined;
            }
        }
  ...
  if (motivo) {
    //AQUI VA EL MOTIVO DE RECHAZO Y EL ENVIO DEL CORREO
    Swal.fire({
          position: 'top-center',
          title: 'Notificando',
          html: '<b>Espere un momento mientras se envía el correo...</b>',
        ...
        $.ajax({
          ...
          url: "send-mailer.php",
          //ACCION 5 SIGNIFICA NOTIFICAR RECHAZO
          //data: "correo="+correo+"&accion=5&motivo="+motivo,
          data: {correo:correo,accion:5,motivo:motivo},
          success: function(resp) {
